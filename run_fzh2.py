...
def get_error(img):
    ...
    num_labels, _, stats, _ = cv2.connectedComponentsWithStats(dilated2, connectivity=8, ltype=None)
    ...
    if len(stats) == 3:
        ...
        if abs(w1 * h1 - w2 * h2) > 40000:
            if w1 * h1 > w2 * h2:
                ...
                i = 0
            dilated2[stats[i][1]:stats[i][1] + stats[i][3], stats[i][0]:stats[i][0] + stats[i][2]] = 0
    # cv2.imshow('dilated3', dilated2)

    # 提取预瞄点
    mid = int(Height / 2)
    _, target = np.where(dilated2[mid - 40:mid + 40, :] > 0)
    bias = target.mean() - int(Width / 2)
...
def recong_img(img):
    ...
    circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 100, param1=100, param2=30, minRadius=5, maxRadius=20)
    ...
    if len(circles) != 0:
        ...
        for row in range(radius * 2):
            for col in range(radius * 2):
                if (row - radius) ** 2 + (col - radius) ** 2 <= (radius - 1) ** 2:
        ...
        _, _, stats, _ = cv2.connectedComponentsWithStats(mask, connectivity=8, ltype=None)
        ...
        # 利用左和右的连接的差异区分
        # result=word_mask[int(word_size/3):,int(word_size/3):]
